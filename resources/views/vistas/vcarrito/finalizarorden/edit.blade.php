								...
								<tr>
									...
									<td>
                                        @if($articulo->oferta_activar == 'NO')
                                            <h5><font color="green">{{ $datosConfig->moneda }}{{number_format($detalle->precio,2, '.', ',')}}</font></h5>
                                        @elseif($articulo->oferta_activar == 'SI') 
                                            <h5><font color="green">{{ $datosConfig->moneda }}{{number_format($detalle->precio,2, '.', ',')}}</font><br> <strike><small><font color="red">{{ $datosConfig->moneda }}{{number_format($articulo->ultimo_precio_venta,2, '.', ',')}}</font></small></strike></h5>
                                        @endif
									</td>
									<td>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @foreach($orden as $ord)
                                        <h5><strong>{{ $datosConfig->moneda }}{{number_format($ord->total,2, '.', ',')}}</strong></h5>
                                    @endforeach
                                </td>
                            </tr>
			...
			<div class="section-top-border">
				<div class="row">
					<div class="col-lg-6 col-md-12">
						...
						{{Form::open(array
						(
							'action' => 'VistaCarritoController@confirmarOrden',
							'method' => 'POST',
							'role' => 'form',
						))
						}}
						{{Form::token()}}
							...
							<div class="input-group-icon mt-10">
								...
								@foreach($orden as $ord)
									<input type="hidden" name="idorden" value="{{$ord->idorden}}">
								@endforeach
							</div>
						{!!Form::close()!!}
					</div>
